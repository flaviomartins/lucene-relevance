...
public abstract class TFIDFSimilarity extends Similarity {
   ...
   * query.  This value is multiplied by the {@link #idf(long, long)}
   * factor for each term in the query and these products are then summed to
   * form the initial score for a document.
   *
   * <p>Terms and phrases repeated in a query indicate the topic of the
   * query, so implementations of this method usually return larger values
   * when <code>freq</code> is large, and smaller values when <code>freq</code>
   * is small.
   *
   * @param freq the frequency of a term within a query
   * @return a score factor based on a term's within-query frequency
   */
  public abstract float qtf(float freq);
  ...
  class TFIDFScorer extends SimScorer {
    ...
    public TFIDFScorer(float boost, Explanation idf, float[] normTable) {
      ...
      this.idf = idf;
      this.boost = boost;
      this.queryWeight = qtf(boost) * idf.getValue().floatValue();
